...
void Cx4::C4DrawWireFrame() {
  ...
  for(int32 i = ram[0x0295]; i > 0; i--, line += 5) {
    if(bus.read(line, 0) == 0xff && bus.read(line + 1, 0) == 0xff) {
      int32 tmp = line - 5;
      while(bus.read(tmp + 2, 0) == 0xff && bus.read(tmp + 3, 0) == 0xff && (tmp + 2) >= 0) { tmp -= 5; }
      point1 = (read(0x1f82, 0) << 16) | (bus.read(tmp + 2, 0) << 8) | bus.read(tmp + 3, 0);
    } else {
      point1 = (read(0x1f82, 0) << 16) | (bus.read(line, 0) << 8) | bus.read(line + 1, 0);
    }
    point2 = (read(0x1f82, 0) << 16) | (bus.read(line + 2, 0) << 8) | bus.read(line + 3, 0);

    X1=(bus.read(point1 + 0, 0) << 8) | bus.read(point1 + 1, 0);
    Y1=(bus.read(point1 + 2, 0) << 8) | bus.read(point1 + 3, 0);
    Z1=(bus.read(point1 + 4, 0) << 8) | bus.read(point1 + 5, 0);
    X2=(bus.read(point2 + 0, 0) << 8) | bus.read(point2 + 1, 0);
    Y2=(bus.read(point2 + 2, 0) << 8) | bus.read(point2 + 3, 0);
    Z2=(bus.read(point2 + 4, 0) << 8) | bus.read(point2 + 5, 0);
    Color = bus.read(line + 4, 0);
